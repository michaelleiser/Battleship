...
public class GameFrame extends JFrame {
	...
	JToggleButton yourField[][];
	JToggleButton enemyField[][];
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < coloumns; j++) {
				if (i == 0 && j == 0) {
					...
					x1.setFont(new Font("Tahoma", 1, 14)); // NOI18N
	...
	private void jButtonReadyActionPerformed(ActionEvent evt) {
		for(int i = 0; i < game.getNbrOfColoumns(); i++){
			for(int j = 0; j < game.getNbrOfRows(); j++){
